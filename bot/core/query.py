...
class Tapper:
    ...
    def __init__(self, query: str = None, session_name: str = None):
        self.query = query
    ...
    async def login(self, http_client: aiohttp.ClientSession) -> None:
        try:
            initdata = self.query
            if not initdata:
                logger.error("Failed to retrieve query. Cannot proceed with login.")
                return None
    ...
    async def run(self, proxy: str | None) -> None:
        ...
        while retry_count < max_retries:
            try:
                ...
                if login_need:
                    self.token = await self.login(http_client=http_client)
...
async def run_query(query: str, session_name: str, proxy: str | None):
    try:
        await Tapper(query=query, session_name=f"{session_name}").run(proxy=proxy)
    except InvalidSession:
        logger.error(f"{session_name} | Invalid Session encountered.")
    except Exception as e:
        logger.exception(f"An unexpected error occurred in session {session_name}: {e}")
